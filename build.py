...
def parse_args():
    parser = argparse.ArgumentParser(description="Fresh-build the libpinyin test programs with vcpkg.")
    parser.add_argument(
        "--vcpkg-prefix",
        type=Path,
...
def main():
    ...
    if vcpkg_host_triplet:
        log(f"Configuring libpinyin manifest for {vcpkg_triplet} with host triplet {vcpkg_host_triplet}")
    else:
        log(f"Configuring libpinyin manifest for {vcpkg_triplet}")
    ...
    log(f"Installed: {install_prefix / 'test_pinyin'}")
    log(f"Installed: {install_prefix / 'test_extract'}")
    log(f"Installed: {install_prefix / 'test_prefix'}")
